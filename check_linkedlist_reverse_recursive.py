import time
class Node:
    def __init__(self,d,n=None):
        self.next=n
        self.data=d
    def getnext(self):
        return self.next
    def getdata(self):
        return self.data
    def setnext(self,next1):
        self.next=next1
    def setdata(self,data1):
        self.data=data1
        
class LinkedList(Node):
    def __init__(self,root1=None):
        self.root=root1
        self.size=0
        
    def addnode(self,data1):
        node=Node(data1)
        node.setnext(self.root)
        self.root=node
        self.size+=1
        start_node=self.root

    def displaylist(self):
        currentnode=self.root
        while currentnode:
            print(str(currentnode.getdata())+" --> ",end="")
            currentnode=currentnode.getnext()

    def reverse_iterative(self):
        current_node=self.root
        prev_node=None
        next_node=current_node.getnext()
        current_node.setnext(prev_node)
        while next_node:
            prev_node=current_node
            current_node=next_node
            next_node=next_node.getnext()
            current_node.setnext(prev_node)

        self.root=current_node





    #not fully implemented
        
    def reverse_recursive(self):
        self._reverse_recursive(self.root)

    def _reverse_recursive(self,current_node):
        if not current_node.getnext():
            return current_node
        else:
            current_node1=current_node.getnext()
            node_temp=self._reverse_recursive(current_node1)
            
        a=current_node.getnext()
        a.setnext(current_node)
        return node_temp

            
           
            
            
            
            
if __name__=="__main__":
    obj1=LinkedList()
    obj1.addnode(20)
    obj1.addnode(30)
    obj1.addnode(10)
    obj1.addnode(20)
    obj1.addnode(10)
    obj1.addnode(10)
    obj1.addnode(20)
    obj1.addnode(50)
    obj1.displaylist()

    print("\nreversing string\n")

    obj1.reverse_recursive()

    print("displaying reverse list")
    obj1.displaylist()
    
    
    
        
        
        
    
        
